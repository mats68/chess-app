...
const OpeningExplorer: React.FC = () => {
  ...
  const handleVariantClick = (variant: ChessVariant, openingName: string, chapterName: string) => {
            ...
            <div ref={provided.innerRef} {...provided.droppableProps}>
              {openings.map((opening, index) => (
                <Draggable key={opening.id} draggableId={opening.id} index={index}>
                  {provided => (
                    <div ref={provided.innerRef} {...provided.draggableProps} className='mb-4 bg-white rounded-lg shadow'>
                      <div className={`flex items-center p-3 border-b ${selectedOpening === opening.id ? 'bg-blue-50' : ''}`}>
                        <div {...provided.dragHandleProps} className='mr-2'>
                          <GripVertical className='w-4 h-4 text-gray-400' />
                        </div>
                        <div className='flex-grow cursor-pointer' onClick={() => setSelectedOpening(selectedOpening === opening.id ? null : opening.id)}>
                              ...
                              <div ref={provided.innerRef} {...provided.droppableProps}>
                                {opening.chapters.map((chapter, chapterIndex) => (
                                  <Draggable key={chapter.id} draggableId={chapter.id} index={chapterIndex}>
                                    {provided => (
                                      <div ref={provided.innerRef} {...provided.draggableProps} className='ml-4 mt-2 bg-gray-50 rounded'>
                                        <div className={`flex items-center p-2 ${selectedChapter === chapter.id ? 'bg-green-50' : ''}`}>
                                          <div {...provided.dragHandleProps} className='mr-2'>
                                            <GripVertical className='w-4 h-4 text-gray-400' />
                                          </div>
                                          <div className='flex-grow cursor-pointer' onClick={() => setSelectedChapter(selectedChapter === chapter.id ? null : chapter.id)}>
                                            {chapter.name}
                                          </div>
                                          <div className='flex space-x-2'>
                                        ...
                                        {selectedChapter === chapter.id && (
                                          <Droppable droppableId={`variants-${opening.id}-${chapter.id}`} type='VARIANT'>
                                            {provided => (
                                              <div ref={provided.innerRef} {...provided.droppableProps} className='ml-8 my-2'>
                                                    ...
                                                    {provided => (
                                                      <div
                                                        ...
                                                        className={`flex items-center p-2 mb-1 rounded cursor-pointer
                                                          hover:bg-purple-50 ${selectedVariant === variant.id ? 'bg-purple-100' : 'bg-white'}`}
                                                        onClick={() => handleVariantClick(variant, opening.name, chapter.name)}>
                                                        <div {...provided.dragHandleProps} className='mr-2'>
                                                          <GripVertical className='w-4 h-4 text-gray-400' />
                                                        </div>
  ...
  return <div className='h-screen bg-gray-50'>{viewMode === 'tree' ? <TreeView /> : <BoardView />}</div>
}
